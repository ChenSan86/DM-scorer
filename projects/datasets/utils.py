...
def read_six_dim_vector(model_name, filelist_dir='data_2.0/filelist'):
    ...
    """
    # 首先尝试从高精度的all_model.txt文件读取
    high_precision_file = '/home/xinguanze/project/ex_2/data_3/all_model.txt'
    if os.path.exists(high_precision_file):
        with open(high_precision_file, 'r', encoding='utf-8') as f:
            for line in f:
                if model_name in line:
                    parts = line.strip().split()
                    # 取6维向量部分（第6-11个元素，索引6-11）
                    try:
                        vector = [float(x) for x in parts[6:12]]
                        return vector
                    except Exception:
                        pass
    
    # 如果高精度文件中找不到，则回退到原来的低精度文件
    file_paths = [

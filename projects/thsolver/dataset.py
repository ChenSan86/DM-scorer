...
def read_six_dim_vector(model_id, result_dir='/home/xinguanze/project/ex_4_dataset_making/result'):
    ...
    try:
        ...
        # 防止除零错误：如果所有值都相同，则返回均匀分布
        if max_val - min_val == 0:
            # 返回均匀分布而不是全零，避免后续计算问题
            normalized = np.full_like(arr, 0.5)
        else:
            normalized = (arr - min_val) / (max_val - min_val)
        
        # 额外的数值检查，确保没有NaN或Inf
        normalized = np.where(np.isnan(normalized) | np.isinf(normalized), 0.5, normalized)
